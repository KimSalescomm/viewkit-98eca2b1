...
export const featuresMap: Record<string, Feature[]> = {
  ...
  vacuum: [
    ...
    },
    {
      ...
      description:
        "청소로봇을 쓸 때 가장 번거로운 게 바닥에 있는 물건을 다 치워두는 일이죠. 깜박하고 못 치운 물건 하나 때문에 청소가 멈춰버리는 일도 종종 생기고요. AlphaUP Drive는 이런 불편을 해결합니다. RGB 카메라와 센서로 장애물을 섬세하게 인식해 최적의 경로로 이동하고, 고립될 위험이 있는 공간은 '청소 금지 구역'으로 스스로 설정해 일단 청소를 끝까지 마쳐요. 그리고 다음 청소 때 그 구역의 장애물이 사라진 것이 확인되면, 빠뜨리지 않고 다시 청소를 재개합니다. 쓸수록 더 똑똑하고 꼼꼼해지는 주행이에요.",
