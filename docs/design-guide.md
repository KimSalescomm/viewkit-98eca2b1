...
## 타이포그래피 / 라벨 스타일 가이드 (공통)

`src/styles/typeScale.ts` 의 상수만 사용합니다. 페이지에서 임의의 px 값을 새로 만들지 않습니다.

| 단계 | 크기/굵기 | 사용처 |
| --- | --- | --- |
| Display | 28px bold | 페이지 메인 타이틀 (예: "구독 케어 전·후 비교") |
| Heading | 18px bold | 섹션 타이틀 (예: "세탁기 케어서비스", "○○ 케어 과정") |
| Body | 14px regular/medium | 부제 설명문, 탭 라벨, 체크리스트 항목, 버튼 텍스트 |
| Caption | 12px | eyebrow(CARE SERVICE), BEFORE/AFTER, 케어 전/후, 이미지 캡션, 디스클레이머 |

컬러 의미 규칙
- 레드 배경 + 흰 텍스트 = 클릭 가능한 액션 (버튼, 탭 선택 상태)
- 레드 텍스트만 = 카테고리/타입 라벨 (CARE SERVICE, AFTER 등)
- 회색 텍스트 = 보조 설명. 회색·얇은 텍스트를 "미포함/비활성" 의미로 쓰지 않습니다.
- 체크리스트 항목은 모두 동일한 색/굵기로 표기하고, 이미지 확인 가능 여부는 레드 "보기" 버튼 유무로만 구분합니다.

배지/버튼 규격
- `actionPill` (rounded-lg, h-7, 12px bold, 레드 배경) = 텍스트 액션 버튼
- `actionIconPill` (rounded-lg, 28x28, 레드 배경) = 아이콘 전용 액션 배지
- 배지 내부 아이콘은 `actionIconSize`(14px)로 통일
