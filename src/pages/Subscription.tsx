...
const _subscriptionProducts: SubscriptionProduct[] = [
  ...
  },
  {
    ...
    careVideo: "https://www.lge.co.kr/kr/main/caresolution/renew_2206/assets/rmsf2025/microwaves_and_ovens_250804.mp4",
    careSteps: [
      { label: "소모품 교체" },
      { label: "토탈 클리닝" },
      { label: "내부 클리닝" },
  ...
  return (
    <div className="min-h-screen bg-[#F3F4F6]">
      <PageContainer as="main" verticalPadding="py-8 sm:py-12">
        ...
        {/* 바스에어시스템: 영상 히어로 + 케어 과정 체크리스트 */}
        {isBathair && (
          (() => {
            const bathairVideoUrl = "https://youtu.be/HkCNqxdZ5bs";
            const { embedUrl } = convertToEmbedUrl(bathairVideoUrl);
            return (
              <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-[0_2px_16px_rgba(0,0,0,0.06)]">
                {bathairPlaying && embedUrl ? (
                  <div className="relative aspect-video bg-black">
                    <iframe
                      src={`${embedUrl}${embedUrl.includes("?") ? "&" : "?"}autoplay=1&mute=1&rel=0&playsinline=1`}
                      title="바스에어시스템 케어 영상"
                  ...
                  <button
                    type="button"
                    onClick={() => setBathairPlaying(true)}
                    className="group block w-full relative aspect-video bg-gray-100 overflow-hidden"
                    aria-label="바스에어시스템 케어 영상 재생"
                  >
                    <img
                      src={youtubeThumbnail(bathairVideoUrl)}
                      alt="바스에어시스템 케어 영상"
                      className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105"
                    />
                    <div
        ...
        <div className="mt-10 pt-6 border-t border-gray-200">
          <ul className="space-y-1.5 text-[12px] leading-relaxed text-gray-700 whitespace-nowrap overflow-x-auto">
            <li className="font-bold text-gray-900">* 이해를 돕기 위해 연출한 광고적 이미지로, 오염 여부 및 실제 케어서비스 결과는 사용 환경에 따라 다를 수 있습니다.</li>
            <li>* 케어서비스 항목은 구독한 제품의 모델, 계약 시점, 구독 타입(프리미엄, 라이트 플러스, 라이트 등)에 따라 다를 수 있으니, 자세한 내용은 구독 계약 시 반드시 확인하세요.</li>
            <li>* 고객 과실로 인한 제품 고장은 무상 A/S에서 제외되며 별도 비용이 발생할 수 있습니다.</li>
